...
import {addRole, changeRoleStatus, deleteRole, getRole, getRolePage, updateRole} from '@/api/role'
  ...
  methods: {
    ...
    getRoleMenuTreeSelect(roleId) {
      ...
      getMenuListByRoleId(roleId).then(response => {
        ...
        let menuIds = data.map(item => item.menuId)
        for (let datum of data) {
          if (menuIds.includes(datum.parentId)) {
            _this.defaultCheckedKeys.push(datum.menuId)
          }
        }
        _this.menuNodeAll = menuIds.length === Object.keys(_this.$refs.menu.store.nodesMap).length
      });
    },
    // 角色状态修改
